...
var OrderFilters = function (_a) {
    ...
    }), regionsPagination = _d[0], setRegionsPagination = _d[1];
    var _e = (0, medusa_react_1.useAdminRegions)(regionsPagination), regions = _e.regions, count = _e.count, isLoadingRegions = _e.isLoading;
    var handlePaginateRegions = function (direction) {
        if (direction > 0) {
            setRegionsPagination(function (prev) { return (__assign(__assign({}, prev), { offset: prev.offset + prev.limit })); });
        }
        else if (direction < 0) {
            setRegionsPagination(function (prev) { return (__assign(__assign({}, prev), { offset: Math.max(prev.offset - prev.limit, 0) })); });
        }
    };
    return (<div className="flex space-x-1">
      <container_1.default submitFilters={onSubmit} clearFilters={onClear} triggerElement={<button className={(0, clsx_1.default)("flex rounded-rounded items-center space-x-1 focus-visible:outline-none focus-visible:shadow-input focus-visible:border-violet-60")}>
        ...
        <item_1.default filterTitle="Regions" options={(regions === null || regions === void 0 ? void 0 : regions.map(function (region) { return ({
            value: region.id,
            label: region.name,
        }); })) || []} isLoading={isLoadingRegions} hasPrev={regionsPagination.offset > 0} hasMore={regionsPagination.offset + regionsPagination.limit < (count !== null && count !== void 0 ? count : 0)} onShowPrev={function () { return handlePaginateRegions(-1); }} onShowNext={function () { return handlePaginateRegions(1); }} filters={tempState.region.filter} open={tempState.region.open} setFilter={function (v) { return setSingleFilter("region", v); }}/>
