...
var reducer = function (state, action) {
    switch (action.type) {
        case "setFilters": {
            return __assign(__assign({}, state), { status: action.payload.status, collection: action.payload.collection, tags: action.payload.tags, date: action.payload.date });
        }
        case "reset": {
...
var useProductFilters = function (existing, defaultFilters) {
    ...
    var reset = function () {
        dispatch({
            type: "setFilters",
            payload: __assign(__assign({}, state), { status: {
                    open: false,
                    filter: null,
                }, collection: {
                    open: false,
    ...
    var getRepresentationObject = function (fromObject) {
        ...
        for (var _i = 0, _a = Object.entries(objToUse); _i < _a.length; _i++) {
            ...
            else if (value === null || value === void 0 ? void 0 : value.open) {
                toQuery[stateFilterMap[key]] = value.filter;
            }
        }
        return toQuery;
