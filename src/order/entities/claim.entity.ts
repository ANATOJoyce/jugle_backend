// order-claim.entity.ts
import { Prop, Schema, SchemaFactory } from '@nestjs/mongoose';
import { Document, Types } from 'mongoose';
import { Order } from './order.entity';
import { OrderClaimItem } from './claim-item.entity';
import { OrderShipping } from './order-shipping-method.entity';
import { Return } from './return.entity';
import { OrderTransaction } from './transaction.entity';

export enum ClaimType {
  REFUND = 'refund',
  REPLACEMENT = 'replacement',
}

type OrderClaimDocument = OrderClaim & Document;

@Schema({
  timestamps: true,
  collection: 'order_claims',
  autoIndex: true,
  toJSON: {
    virtuals: true,
    transform: (doc, ret) => {
      ret.id = `claim_${doc._id.toString()}`;
      delete ret._id;
      delete ret.__v;
      return ret;
    },
  },
  id: false, // Désactive le virtual getter 'id' par défaut
})
export class OrderClaim {
  @Prop({ type: Number, required: true })
  order_version: number;

  @Prop({ type: Number, unique: true })
  display_id: number;

  @Prop({ type: String, enum: ClaimType, required: true })
  type: ClaimType;

  @Prop({ type: Boolean, required: false })
  no_notification?: boolean;

  @Prop({ type: Number, required: false })
  refund_amount?: number;

  @Prop({ type: String, required: false })
  created_by?: string;

  @Prop({ type: Date, required: false })
  canceled_at?: Date;

  @Prop({ type: Object, required: false })
  metadata?: Record<string, any>;

  @Prop({ type: Types.ObjectId, ref: 'Order', required: true, index: true })
  order: Types.ObjectId | Order;

  @Prop({ type: Types.ObjectId, ref: 'Return', required: false, index: true })
  return?: Types.ObjectId | Return;

  @Prop({ type: [{ type: Types.ObjectId, ref: 'OrderClaimItem' }], default: [] })
  additional_items: Types.ObjectId[] | OrderClaimItem[];

  @Prop({ type: [{ type: Types.ObjectId, ref: 'OrderClaimItem' }], default: [] })
  claim_items: Types.ObjectId[] | OrderClaimItem[];

  @Prop({ type: [{ type: Types.ObjectId, ref: 'OrderShipping' }], default: [] })
  shipping_methods: Types.ObjectId[] | OrderShipping[];

  @Prop({ type: [{ type: Types.ObjectId, ref: 'OrderTransaction' }], default: [] })
  transactions: Types.ObjectId[] | OrderTransaction[];

  @Prop({ type: Date, required: false, index: true })
  deleted_at?: Date;

  // Déclaration explicite pour TypeScript
  _id: Types.ObjectId;
  id: string;
}

export const OrderClaimSchema = SchemaFactory.createForClass(OrderClaim);

// Configuration des virtuals
OrderClaimSchema.virtual('id').get(function (this: OrderClaimDocument) {
  return `claim_${this._id.toString()}`;
});

// Middleware pour le cascading delete
OrderClaimSchema.pre('deleteOne', { document: true }, async function () {
  await Promise.all([
    this.model('OrderClaimItem').deleteMany({ 
      $or: [
        { claim: this._id, is_additional_item: true }, // additional_items
        { claim: this._id, is_additional_item: false } // claim_items
      ]
    }),
    this.model('OrderTransaction').deleteMany({ claim: this._id }),
  ]);
});

// Configuration des index
OrderClaimSchema.index(
  { display_id: 1 },
  {
    name: 'IDX_order_claim_display_id',
    partialFilterExpression: { deleted_at: { $ne: null } },
  },
);

OrderClaimSchema.index(
  { deleted_at: 1 },
  {
    name: 'IDX_order_claim_deleted_at',
    partialFilterExpression: { deleted_at: { $ne: null } },
  },
);

OrderClaimSchema.index(
  { order: 1 },
  {
    name: 'IDX_order_claim_order_id',
    partialFilterExpression: { deleted_at: { $ne: null } },
  },
);

OrderClaimSchema.index(
  { return: 1 },
  {
    name: 'IDX_order_claim_return_id',
    partialFilterExpression: { 
      return: { $ne: null },
      deleted_at: { $ne: null } 
    },
  },
);
/*

// Auto-increment pour display_id/*
OrderClaimSchema.pre('save', async function (next) {
  if (this.isNew) {
    const lastClaim = await this.constructor.findOne({}, {}, { sort: { display_id: -1 } });
    this.display_id = lastClaim ? lastClaim.display_id + 1 : 1;
  }
  next();
});*/